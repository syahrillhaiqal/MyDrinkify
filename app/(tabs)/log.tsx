...
const Log = () => {
        ...
        if (selectedPeriod === "daily") {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const totalVolume = dayLogs.reduce(
                    (sum, log) => sum + calculateLogVolume(log),
                    0
                );
    ...
    const loadChartData = async () => {
        ...
        try {
            ...
            const logs = await getWaterLogsByDateRange(
                user.$id,
                startDateStr,
                endDateStr
            );
    ...
    const loadDailyData = async () => {
        ...
        try {
            // Load water types
            const types = await getUserWaterTypes(user.$id);
            setWaterTypes(types as WaterType[]);

            // Load water logs for selected date
            const formattedDate = formatDate(selectedDate);
            const logs = await getWaterLogsByDate(user.$id, formattedDate);
    ...
    useFocusEffect(
        useCallback(() => {
            loadChartData();
            loadDailyData();
        }, [user.$id, selectedPeriod, selectedDate])
    );
                        ...
                        <View className="bg-white w-full mt-2 p-3 rounded-xl h-[350px] border border-gray-200 mb-[100px]">
                            ...
                            >
                                {loading ? (
                                    ...
                                    <View className="flex-1 justify-center items-center">
                                        <Text className="text-gray-500">
                                            No water logs for this date
                                        </Text>
                                    </View>
                                ) : (
                                    waterLogs.map((log) => {
                                        const waterType =
                                            getWaterTypeFromLog(log);
                                        const totalVolume = waterType
                                            ? waterType.volume * log.quantity
                                            : 0;

                                        return (
                                            <View
                                                key={log.$id}
                                                className="flex-row items-center justify-between mb-3 p-2 bg-gray-50 rounded-lg border border-gray-100"
                                            >
                                                {/* Icon */}
                                                <Svg
                                                    height={40}
